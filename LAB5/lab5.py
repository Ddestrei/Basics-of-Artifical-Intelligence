...
class CNN_V1:
    def __init__(self):
        ...
        self.output_weights = np.random.uniform(high=0.1, low=-0.1, size=(10, 17576)) # size=(10, 10816)
    ...
    def seg_and_conv(self,image, filters):
        ...
        for f in filters:
            f = f.T
            filters_size = f.shape[0]
            for x in range(0,image.shape[1]-f.shape[1]+1, 1):
                row = []
                for y in range(0,image.shape[0]-f.shape[0]+1, 1):
                    segment = image[y:y+filters_size, x:x+filters_size].flatten()
                    sum = np.sum(segment * f.flatten())
                    row.append(sum)
                row = np.array(row)
    ...
    def train(self, img, goal):
        ...
        error = np.sum(layer_output - goal)**2
    ...
    def fit(self, input_data, output_data, n_epoch):
        for i in range(n_epoch):
            for j in range(input_data.shape[0]):
                error = self.train(input_data[j,:,: ], output_data[j,:])
                if j % 10 == 0:
                    print(error)
...
cnn_v1_1 = CNN_V1()
cnn_v1_1.fit(train_images[:300],train_labels_new[:300],2)
#cnn_v1_1.train(train_images[0],train_labels_new[0])
print('RUN TEST')
print(cnn_v1_1.test(test_images[:100],test_labels_new[:100]))
